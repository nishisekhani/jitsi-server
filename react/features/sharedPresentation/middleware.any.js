...
import { resetSharedVideoStatus, setSharedVideoStatus } from "./actions.any";
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    ...
    const { ownerId: stateOwnerId, videoUrl: statevideoUrl } =
        ...
        if (conference && conference !== previousConference) {
            conference.addCommandListener(
                ...
                ({ value, attributes }) => {
                    ...
                    const localParticipantId = getLocalParticipant(
                        getState()
                    ).id;
                    const status = attributes.state;

                    if (isSharingStatus(status)) {
                        handleSharingVideoStatus(
                            store,
                            value,
                            attributes,
                            conference
 ...
 * @returns {void}
 */
function handleSharingVideoStatus(
    store,
    videoUrl,
    { state, time, from, muted },
    conference
) {
    const { dispatch, getState } = store;
    ...
    if (
        state === "start" ||
        !["playing", "pause", "start"].includes(oldStatus)
    ) {
        const youtubeId = videoUrl.match(/http/) ? false : videoUrl;
        const avatarURL = youtubeId
 ...
 * @returns {void}
 */
function sendShareVideoCommand({
    id,
    status,
    conference,
    localParticipantId,
